...
std::string key;
int tone_idx[6] = {0}; // The indices (0 - 12) of the waveform periods that the
                       // are being selected to play in the audio
std::vector<std::vector<std::vector<uint32_t>>>
    waveform_luts; // the waveform lut stores the insturment waveforms.
int nok = 0;       // No. of keys currently being presses simutaneously
int instru = 0;    // Instrument idx, current there are 4 instruments
// Recodes whether a key is being presses. This filters out the pressed keys and
// enables the key to be detected in the order of their presses
bool press_list[12] = {false};

const uint32_t frequencies[12] = {262, 277, 294, 311, 330, 349,
                                  370, 392, 415, 440, 466, 494};
std::string keystrings[12] = {"C",  "C#", "D",  "D#", "E",  "F",
                              "F#", "G",  "G#", "A",  "A#", "B"};
uint32_t Ts[13] = {};
...
void scanKeysFunction(uint8_t *TX_Message) {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (!HAL_GPIO_ReadPin(GPIOA, key_cols[i % 4]) && !press_list[i]) {
      ...
      TX_Message[0] = 'P';
      TX_Message[2] =
          i; // Assign number of note played to 3rd entry in transmission
  ...
  // setRow(4);
  // delayMicroseconds(3);
  // decayKnob.updateRotation(std::to_string(!HAL_GPIO_ReadPin(GPIOA,
  // GPIO_PIN_8)) + 	std::to_string(!HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_3)));
}
  ...
  while (1) {
    ...
    for (uint32_t writeCtr = 0; writeCtr < SAMPLE_BUFFER_SIZE / 2; writeCtr++) {
      ...
      if (nok != 0) {
        ...
        // Ts = the 13 periods of the keys, the first period is 1 corresponding
        // to no keys instru = Selects the instrument, currrent is 0 - 3
        uint32_t instru = instrumentKnob.getRotationISR();
        // Vout = 0;
        // for (int i = 0; i < sizeof(tone_idx) / sizeof(tone_idx[0]); i++) {
        //   Vout += waveform_luts[instru][tone_idx[i]][(t % Ts[tone_idx[i]])] *
        //           decay[i];
        // }
        // Vout = nok > 1 ? Vout / nok : Vout;
        Vout = (waveform_luts[instru][tone_idx[0]][(t % Ts[tone_idx[0]])] *
                    decay[0] +
                waveform_luts[instru][tone_idx[1]][(t % Ts[tone_idx[1]])] *
                    decay[1] +
                waveform_luts[instru][tone_idx[2]][(t % Ts[tone_idx[2]])] *
                    decay[2] +
                waveform_luts[instru][tone_idx[3]][(t % Ts[tone_idx[3]])] *
                    decay[3] +
                waveform_luts[instru][tone_idx[4]][(t % Ts[tone_idx[4]])] *
                    decay[4] +
                waveform_luts[instru][tone_idx[5]][(t % Ts[tone_idx[5]])] *
                    decay[5]) /
               std::max(nok, 1); // Divide the
        // amplitude by the totoal number of keys being pressed
      ...
      // If no keys are being pressed the timer resets along with the decay
      // factors
      else {
        t = 0;
        for (int i = 0; i < sizeof(decay) / sizeof(decay[0]); i++) {
          decay[i] = 1.0;
        }
        // decay[0] = 1;
        // decay[1] = 1;
...
void setup() {
  ...
  std::cout << "Key scanning task latent execution time: "
  ...
  std::cout << "Display update task latent execution time: "
            << (micros() - startTime) / 32 << " microseconds" << std::endl;
  ...
  // 	CAN_TX_Function(msgOut);
  // }
  // std::cout << "Data transmission task latent execution time: " <<
  // (micros()-startTime) / 1 << " microseconds" << std::endl;
  ...
  // 	CAN_RX_Function(local_RX);
  // }
  // std::cout << "Data receiving task latent execution time: " <<
  // (micros()-startTime) / 1 << " microseconds" << std::endl;
  ...
  std::cout << "DAC ISR task latent execution time: "
            << (micros() - startTime) / 32 << " microseconds" << std::endl;
