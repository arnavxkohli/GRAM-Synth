...
extern int keynum;
extern int tone_idx[4]; // The indices (0 - 12) of the waveform periods that the are being selected to play in the audio
extern std::vector<std::vector<std::vector<uint16_t>>> waveform_lut; // the waveform lut stores the insturment waveforms.
...
extern const uint16_t frequencies[36];
extern std::string keystrings[36];
extern uint16_t Ts[37];
